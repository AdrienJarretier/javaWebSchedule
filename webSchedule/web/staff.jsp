...
<html>
    ...
    <body>
        <h1>Welcome Mr. ${staff.name}</h1>
        Here are all lessons : 
        <table border="1">
            <tr><th>Id</th><th>Time_start</th><th>Time_end</th><th>Title</th><th>Class_room</th><th>Teacher</th></tr>
            <c:forEach var="lesson" items="${lessons}">
                <tr><td>${lesson.id}</td><td>${lesson.time_start}</td><td>${lesson.time_end}</td><td>${lesson.title}</td><td>${lesson.class_room_id}</td><td>${lesson.teacher_id}</td></tr>
            </c:forEach>		
        </table>
